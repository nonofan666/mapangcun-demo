const pageMeta={dashboard:['经营概览','掌握平台交易、履约和商家经营情况'],custom:['定制需求','承接用户从专题页提交的个性化出行需求'],orders:['订单与核销','统一处理支付订单、履约核销与售后'],revenue:['收益明细','查看平台收益、商家应收与结算进度'],merchants:['商家管理','管理多商家入驻、经营与账号状态'],qualification:['资质审核','审核商家主体证照和行业经营许可'],products:['商品管理','管理全平台商品、价格、库存和上架审核'],rules:['规则中心','统一管理用户端展示、预约库存、退款和履约参数']};
const demands=[
...
function bindRows(){
 ...
 document.querySelectorAll('.order-detail').forEach(b=>b.onclick=()=>openModal('订单详情',b.dataset.id,`<div class="detail-grid"><div><span>联系人 / 手机</span><b>周女士 · 138****2806</b></div><div><span>出行人数</span><b>成人 2 · 儿童 1</b></div><div><span>预约日期</span><b>2026-08-29</b></div><div><span>发票信息</span><b>电子普票 · 个人</b></div><div><span>支付状态</span><b>已支付</b></div><div><span>履约状态</span><b>待使用</b></div><div><span>核销方式</span><b>统一二维码</b></div><div><span>平台佣金</span><b>售价 × 3%</b></div></div><div class="selected-list"><div><b>特殊需求</b><span>订单备注</span></div><div><span>饮食忌口</span><b>儿童不吃辣</b></div><div><span>其他需求</span><b>带老人，希望节奏舒缓</b></div><div><b>履约明细</b><span>平台可查看全部</span></div><div><span>马胖村</span><b>待核销</b></div></div>`,'关闭'));
...
function productForm(title,name=''){openModal(title,'商品将由平台审核后上架',`<div class="form-grid"><label class="wide">商品名称<input value="${name}" placeholder="请输入商品名称"></label><label>商品类目<select><option>茶叶</option><option>蔬菜水果</option><option>单项体验</option><option>住宿</option><option>餐饮</option><option>交通</option></select></label><label>所属商家<select><option>马胖村</option></select></label><label>销售价格<input placeholder="¥ 0.00"></label><label>库存数量<input type="number" value="20"></label><label>库存单位<select><option>盒</option><option>袋</option><option>斤</option><option>份</option></select></label><label>平台佣金<input value="3%"></label><label class="wide">商品说明<textarea placeholder="按类目填写规格、产地、配送范围和售后说明"></textarea></label><label class="wide">预约设置<select><option>全年可预约，结算时确认日期</option><option>指定日期范围</option><option>实物商品，无需预约</option></select></label></div>`,'提交审核',()=>toastMsg('商品已提交平台审核'))}
...
verifyBtn.onclick=()=>openVerify();addProduct.onclick=()=>productForm('新增商品');newDemand.onclick=()=>openModal('录入线下定制需求','适用于电话、微信或到店咨询',`<div class="form-grid"><label>联系人<input placeholder="姓名"></label><label>联系电话<input placeholder="手机号"></label><label>出行日期<input type="date"></label><label>成人数量<input type="number" value="2"></label><label>儿童数量<input type="number" value="0"></label><label>预算范围<input placeholder="如 ¥5,000—8,000"></label><label>意向套餐<select><option>暂不确定</option><option>2天1晚</option><option>3天2晚</option><option>5天4晚</option></select></label><label>发票需求<select><option>暂不需要</option><option>电子普票</option><option>增值税专票</option></select></label><label class="wide">特殊需求<textarea placeholder="住宿、体验、用车、饮食忌口、老人儿童照护等"></textarea></label></div>`,'保存并分派',()=>toastMsg('定制需求已录入'));
...
roleSwitch.onclick=()=>openModal('切换后台视角','当前登录账号：稚旅超级管理员',`<div class="selected-list"><div><span>稚旅平台运营端</span><b>最高权限 · 当前视角</b></div><div><span>马胖村商家端</span><b>预览商家权限 →</b></div></div><div class="notice" style="margin-top:14px"><span>商家端仅可查看和处理归属马胖村的商品、库存、订单、核销、收益及资质。</span></div>`,'切换为商家视角',()=>toastMsg('已切换至马胖村商家端预览'));
document.querySelectorAll('.rule-edit').forEach(b=>b.onclick=()=>openModal('编辑业务规则','原型交互：保存后同步到用户端',`<div class="form-grid"><label class="wide">规则名称<input value="${b.closest('.rule-card').querySelector('h2').textContent}"></label><label class="wide">规则说明<textarea>当前规则已按专题页最新产品方案配置，可在正式开发中接入接口和权限审批。</textarea></label><label>生效范围<select><option>全部商家</option><option>指定商家</option></select></label><label>生效时间<select><option>保存后立即生效</option><option>定时生效</option></select></label></div>`,'保存并发布',()=>toastMsg('规则已保存并同步到用户端')));
saveRules.onclick=()=>{localStorage.setItem('mp_admin_rules_saved',new Date().toISOString());toastMsg('全部规则已保存并发布')};
