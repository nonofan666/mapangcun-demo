...
function productForm(title,name=''){openModal(title,'商品将由平台审核后上架',`<div class="form-grid"><label class="wide">商品名称<input value="${name}" placeholder="请输入商品名称"></label><label>商品类目<select><option>单项体验</option><option>住宿</option><option>餐饮</option><option>交通</option><option>农产品</option></select></label><label>所属商家<select><option>茶禾境教基地</option><option>马胖村云上民宿</option></select></label><label>销售价格<input placeholder="¥ 0.00"></label><label>每日库存<input type="number" value="20"></label><label class="wide">商品说明<textarea placeholder="按类目填写面积/设施服务，或内容简介/购买须知"></textarea></label><label class="wide">预约设置<select><option>全年可预约，结算时确认日期</option><option>指定日期范围</option></select></label></div>`,'提交审核',()=>toastMsg('商品已提交平台审核'))}
